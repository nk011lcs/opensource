...
class MinHash(object):
    ...
    def __init__(self, num_perm=128, seed=1,
            ...
            hashvalues=None, permutations=None, size=32):
        self.max_hash = np.uint64((1 << size) - 1)
        self.hash_range = (1 << size)
        ...
        if num_perm > self.hash_range:
            ...
            raise ValueError("Cannot have more than %d number of\
                    permutation functions" % self.hash_range)
        self.seed = seed
    ...
    def update(self, b):
        ...
        phv = np.bitwise_and((a * hv + b) % _mersenne_prime, self.max_hash)
    ...
    def update_batch(self, b):
        ...
        a, b = self.permutations
        phv = np.bitwise_and(((hv * np.tile(a, (len(hv), 1)).T).T + b) % _mersenne_prime, self.max_hash)
    ...
    def count(self):
        ...
        '''
        k = len(self)
        return np.float(k) / np.sum(self.hashvalues / np.float(self.max_hash)) - 1.0
